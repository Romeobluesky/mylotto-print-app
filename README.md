...
- **수동입력**: 파일 없이 별도 창에서 번호를 직접 입력해 조합 추가. 1~6개 부분
...
1. 앱 실행 → **‘파일 열기’** 로 조합 파일 선택 (또는 **‘수동입력’** 으로 직접 입력 — 아래 참고)
2. 좌측 패널에서 인쇄할 조합 체크 (전체 선택 가능)
3. **‘인쇄 설정’** 에서 프린터 선택 + 시험 인쇄로 위치 확인
4. **‘인쇄 시작’** → 5조합씩 한 페이지로 자동 출력

### 2-1. 수동입력 (파일 없이 직접 추가)

오른쪽 버튼 패널의 **‘수동입력’** 을 누르면 별도 창이 열립니다.
...
- 행 삭제: 행 우측 휴지통 버튼
- **‘확인’** 누르면 조합 리스트 최상단에 추가되고 우측에 `[수동입력]` 표시
...
src/                  Renderer (React)
  components/
    ...
    ManualInputView.tsx  수동입력 별도 창 (반자동 지원)
