...
🔗 **공식 사이트**: [winspot.kr](https://winspot.kr) — 로또 번호 분석·추천 서비스. 이
앱은 WinSpot 에서 생성한 추천 조합을 실제 OMR 용지에 출력하는 도구로 함께 사용
됩니다.
...
- **수동 입력**: 파일 없이 별도 창에서 번호를 직접 입력해 조합 추가. 1~6개 부분
  입력 시 반자동으로 자동 처리 (방향키 칸 이동 / `+` 버튼 Enter 로 행 추가)
- **그룹 번호 배지**: 파일 조합을 5행 단위로 묶어 우측에 번호 (`1`, `2`, `3`…) 표시.
  배지 클릭 시 해당 5행이 하이라이트되어 인쇄된 OMR 용지와 대조해 문제 시트를
  바로 식별 가능. 수동 입력 항목은 별도 `M` 배지로 묶여 파일 그룹 번호 정렬을 방해하지 않음
- **선택 인쇄**: 체크한 조합만 인쇄. 전체선택 체크박스 한 번으로 일괄 선택, 삭제
  시 확인 다이얼로그로 실수 방지
...
1. 앱 실행 → **‘파일 열기’** 로 조합 파일 선택 (또는 **‘수동 입력’** 으로 직접 입력 — 아래 참고)
2. 좌측 리스트에서 인쇄할 조합 체크 — **‘전체선택’** 체크박스로 한 번에 선택 가능
3. **‘인쇄 설정’** 에서 프린터 선택 + 시험 인쇄로 위치 확인
4. **‘선택 인쇄’** → 체크한 조합만 5개씩 한 페이지로 자동 출력
5. 출력 후 셀에서 벗어난 마킹이 보이면 우측 그룹 번호 배지 클릭으로 해당 5행
   하이라이트 → 어느 시트가 문제인지 한눈에 식별

### 2-1. 수동 입력 (파일 없이 직접 추가)

오른쪽 버튼 패널의 **‘수동 입력’** 을 누르면 별도 창이 열립니다.

- 한 행에 1~45 사이 서로 다른 숫자를 **1~6개** 입력
- 6개 미만이면 **반자동** 으로 자동 처리 (해당 게임 칸의 반자동 체크박스가 함께 마킹됨)
- 칸 이동: <kbd>←</kbd> <kbd>→</kbd> 방향키
- 행 추가: 행 우측 `+` 버튼 클릭 또는 `+` 에 포커스 둔 채 <kbd>Enter</kbd>
- 행 삭제: 행 우측 휴지통 버튼
- **‘확인’** 누르면 조합 리스트 최상단에 추가되고 우측에 단일 `M` 배지로 묶여 표시
  (개수에 무관하게 하나의 그룹으로 표시되어 아래 파일 그룹의 5행 번호 정렬이
  유지됨)
...
src/                  Renderer (React)
  components/
    ...
    ManualInputView.tsx  수동 입력 별도 창 (반자동 지원)
